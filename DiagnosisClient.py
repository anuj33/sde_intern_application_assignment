...
class DiagnosisClient:
    ...
    def _loadToken(self, username, password, url):
        rawHashString = hmac.new(bytes(password).encode('utf-8'), url.encode('utf-8')).digest()
    ...
    def loadMedicalIssues(self, selectedSymptoms, gender, yearOfBirth):
        if not selectedSymptoms:
            raise ValueError("selectedSymptoms can not be empty")
        if not gender:
            return {"error" : "gender can not be empty" }
        if not yearOfBirth:
            return {"error" : "birth year cannot be empty" }
    ...
    def loadProposedSymptoms(self, symptomName, gender, yearOfBirth):
        if not symptomName:
            return {"error" : "symptom name can not be empty" }
        if not gender:
            return {"error" : "gender can not be empty" }
        if not yearOfBirth:
            return {"error" : "birth year cannot be empty" }

        symptoms_list = self.loadSymptoms()
        symptom_ids = []
        for symptom_dict in symptoms_list:
            if(symptom_dict.get('Name') == symptomName):
                symptom_ids.append(symptom_dict.get("ID"))


        if(len(symptom_ids)  == 0):
            return {"error" : "Symptom not found in priaid database" }

        serializedSymptoms = json.dumps(symptom_ids)
